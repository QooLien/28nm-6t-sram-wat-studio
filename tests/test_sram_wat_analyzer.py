...
class AnalyzerTests(unittest.TestCase):
    ...
    def test_combined_summary_comparison_reads_two_files(self):
        with tempfile.TemporaryDirectory() as td:
            ...
            self.assertIn("RSNM ranking @ 0.70 V", svg)
            self.assertIn("BL Write Margin ranking @ 0.70 V", svg)
    ...
    def test_multi_curve_uses_axis_ticks_instead_of_point_labels(self):
        ...
        self.assertNotIn('class="multi-lot-data-label"', svg)
        self.assertNotIn('class="multi-lot-data-label-bg"', svg)
        self.assertIn("RSNM ranking @ 0.80 V", svg)
        self.assertIn("BL Write Margin ranking @ 0.80 V", svg)
