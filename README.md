...
To combine a completed Multi-VDD run, use **Import Multi-VDD Output Folder...** in the Estimate Vmin tab and select the run folder created by the multi-sheet Excel workflow, then choose **Create Comparison View**. The tool reads every `multi_cell_by_vdd/Model_VDD_*/multi_chip_snm_summary.csv` in that selected folder and combines the measured voltage points into RSNM and BL Write Margin panels; WSNM is excluded from Comparison View. One folder is sufficient; additional folders can be added for overlay comparison. The chart legend ranks Lot/Wafer sources using the mean across shared VDDs of the weaker RSNM and BL-margin percentile. When a higher-VDD minimum is worse than the adjacent lower-VDD minimum, the report compares the two Cells that produced those exact curve points, adds P10/median population context, and—when side-specific six-MOS WAT data is available—performs a common-VDD one-MOS-at-a-time replacement sensitivity to identify the strongest MOS candidate. Detailed outputs include `estimate_vmin_lot_wafer_ranking.csv` and `estimate_vmin_worst_cell_mos_attribution.csv`. The report also adds exact worst-Cell tables and VDD-grouped Tukey box plots for RSNM, BL Write Margin, CR, PR and PUL/PUR/PGL/PGR/PDL/PDR Idsat. This is compact-model screening rather than silicon sign-off.
