...
The Estimate Vmin import accepts generated `multi_chip_snm_summary.csv` files and raw 6T Multi-Cell Excel workbooks (`.xlsx` / `.xlsm`). Excel rows are read from the same Lot/Wafer, Chip ID, Model VDD and six-MOS Vt/Idsat schema used by **Import Multi-Cell**, then evaluated with the current GUI Model Settings before entering the common summary path. Selecting one source file automatically enters **Shmoo-only mode** and outputs only the CR/PR Shmoo, Advisor, statistics, source backup and related CSV files, even if that file contains multiple VDD groups. Estimate Vmin trend images, eye-closure estimation and `multi_chip_snm_summary_combined.csv` are generated only when two or more source files with at least two total Model VDD points are selected.
