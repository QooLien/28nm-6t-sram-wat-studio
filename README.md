...
To combine a completed Multi-VDD run, use **Import Multi-VDD Output Folder...** in the Estimate Vmin tab and select the run folder created by the multi-sheet Excel workflow, then choose **Create Comparison View**. The tool reads every `multi_cell_by_vdd/Model_VDD_*/multi_chip_snm_summary.csv` in that selected folder and combines the measured voltage points into one Read/Write SNM plus BL Write Margin Trip chart. One folder is sufficient; additional folders can be added for overlay comparison. **Import Combined Summaries...** remains available for legacy `multi_chip_snm_summary_combined.csv` exports. The report exports responsive HTML plus PNG, SVG and CSV files. The X axis automatically spans the lowest through highest imported Model VDD, so the curves do not retain an empty 0 V region. Dense point-value labels are omitted from the trend chart; an exact lookup table instead lists the limiting Read SNM, Write SNM and BL Write Margin Cell at each source/VDD together with CR, PR and six-MOS Vt/Idsat. The report also adds VDD-grouped Tukey box plots for RSNM, BL Write Margin, CR, PR and PUL/PUR/PGL/PGR/PDL/PDR Idsat; every box includes a centered median-value label. New Multi-Cell summaries retain side-specific six-MOS WAT values; legacy combined summaries remain readable but cannot reconstruct distributions that were not exported. Select the Multi-VDD output folder or rerun the raw workbook when complete box plots are required. Read/Write SNM share the upper trend panel, and the extrapolated eye-closure segment is intentionally omitted.
