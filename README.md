...
New Multi-Cell summaries also retain each cell's family-average PU/PG/PD Vt and Idsat. When these summaries are imported in **Estimate Vmin Curve**, the report creates one empirical drive-balance shmoo per Model VDD. Its X axis is `beta_PG/beta_PU = PR` (higher/right improves write ability), while Y is `beta_PD/beta_PG = CR` (higher/up improves read drive), so the preferred improvement direction is upper-right. A red/green nearest-measured-cell background visualizes the current data-supported weak/preferred regions; it is relative screening, not silicon Pass/Fail. The same page includes PU, PG and PD Vt-versus-Idsat distributions, the weakest-cell arrow toward the median preferred center, and per-cell Vt/Idsat target deltas in `estimate_vmin_cr_pr_shmoo.csv`. The balanced maximin score is the smaller of normalized RSNM (read stability) and BL Write Trip Margin (write ability) at the same VDD; residual WSNM remains a separate write-eye diagnostic because its direction is not equivalent to larger-is-better RSNM. Purple-ring points are within 90% of the best score. Older summaries remain importable, but can show only the ratio panel when they do not contain family WAT fields.

To compare completed runs, use **Add Comparison Folder...** in the Estimate Vmin tab once for each of two or more output folders, then select **Compare 2+ Lot/Wafer Curves**. The tool reads `estimate_vmin_cr_pr_shmoo.csv` (or the legacy `multi_chip_snm_summary_combined.csv` fallback), groups rows by Lot/Wafer name, and overlays Read SNM, Write SNM and BL Write Margin versus Model VDD. The comparison exports responsive HTML plus PNG, SVG and CSV files.
