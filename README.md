...
The Estimate Vmin import accepts generated `multi_chip_snm_summary.csv` files and raw 6T Multi-Cell Excel workbooks (`.xlsx` / `.xlsm`). For a one-step multi-voltage run, name the data worksheets by Model VDD, for example `0.90V`, `0.80V`, or `VDD_0.70V`; the sheet-name voltage overrides any copied Model VDD column. The program first runs the complete Multi-Cell analysis for every sheet and saves it under `multi_cell_by_vdd/Model_VDD_x.xxxV/`, then combines every VDD population into the Read SNM, Write SNM and BL Write Margin Estimate Vmin curves under `estimate_vmin/`. Conventional single-sheet Excel and a single CSV source retain **Shmoo-only mode**. Multi-Cell run folders and imported Excel backup names include the analyzed VDD for easier identification.

To compare completed runs, use **Import Combined Summaries...** in the Estimate Vmin tab and select two or more generated `multi_chip_snm_summary_combined.csv` files, then choose **Create Comparison View**. Each selected file becomes one Lot/Wafer curve set; the tool no longer scans output folders or substitutes another export. The Comparison View overlays Read SNM, Write SNM and BL Write Margin versus Model VDD and exports responsive HTML plus PNG, SVG and CSV files. In the stacked view, every measured VDD has one solid guide and voltage label per panel; Read/Write SNM share the upper-panel guide, and the extrapolated eye-closure segment is intentionally omitted.
